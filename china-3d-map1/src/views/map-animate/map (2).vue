<template>
  <div class="map-level">
    ...
    <!-- 添加岗位推荐组件 -->
    <job-recommendation ref="jobRecommendationRef"></job-recommendation>
    
    <!-- 添加岗位预警组件 -->
    <job-alert ref="jobAlertRef"></job-alert>
    ...
    <!-- 左上饼图 -->
    <div class="table left-table top" v-show="chartVisible.pie && !is3DMap">
    ...
    <!-- 左下词云图 -->
    <div class="table left-table bottom" v-show="chartVisible.wordCloud && !is3DMap">
      <div class="chart-header">
        <span>地区分布</span>
      </div>
    ...
    <div class="map-btn-group" v-show="app && app.currentScene === 'mainScene'">
      <div class="btn" :class="{ active: showPolicyMatcherModal }" @click="openPolicyMatcher">政策推荐</div>
      <div class="btn" :class="{ active: showCareerTestModal }" @click="openCareerTest">职业测试</div>
      <div class="btn" :class="{ active: is3DMap }" @click="toggleMapMode">3D地图户</div>
      ...
      <div class="btn" :class="{ active: state.card }" @click="setEffectToggle('card')">标签</div>
      <div class="btn" :class="{ active: state.particle }" @click="setEffectToggle('particle')">粒子效果</div>
      <div class="btn" :class="{ active: state.mirror }" @click="setEffectToggle('mirror')">镜面反射</div>
      <div class="btn" :class="{ active: state.path }" @click="setEffectToggle('path')">路径</div>
      <!-- 添加控制图表显示的按钮 -->
      <div class="btn" :class="{ active: chartVisible.pie }" @click="toggleChart('pie')">学历图</div>
      <div class="btn" :class="{ active: chartVisible.wordCloud }" @click="toggleChart('wordCloud')">地区图</div>
      <div class="btn" :class="{ active: chartVisible.calendar }" @click="toggleChart('calendar')">趋势图</div>
    </div>
...
const drawPieChart = async () => {
  try {
    ...
    setTimeout(async () => {
      ...
      try {
        // 读取 CSV 文件
        const response = await fetch('/result_processed1.csv');
        const csvText = await response.text();
        
        // 解析 CSV 数据
        const rows = csvText.trim().split('\n').slice(1); // 跳过标题行
        const data = rows.map(row => {
          const [newNeed, count] = row.split(',');
          return {
            name: newNeed,
            value: parseInt(count.trim())
          };
        }).filter(item => item.name && !isNaN(item.value)); // 过滤无效数据
...
const drawCalendarChart = async () => {
  try {
    ...
    setTimeout(async () => {
      ...
      try {
        // 读取 CSV 文件
        const response = await fetch('/河流图200.csv');
        if (!response.ok) {
        ...
        // 提取所有唯一的职位名称
        const allNames = [...new Set(riverData.map(item => item.name))];
        ...
        const series = allNames.map(name => {
          const data = sortedDates.map(date => {
            const matchingItems = dateGroups[date].filter(item => item.name === name);
            return [date, matchingItems.length > 0 ? matchingItems[0].value : 0];
          });
        ...
        const option = {
          ...
          legend: {
            data: allNames.slice(0, 10).map(name => name.length > 10 ? name.substring(0, 10) + '...' : name),
            textStyle: {
              color: '#fff',
              fontSize: 10
            },
          ...
          series: series.slice(0, 10) // 限制显示前10个职位类别，避免过于复杂
        };
        ...
        console.error('Error loading or parsing river chart CSV:', error);
      }
    }, 100);
  } catch (error) {
...
const drawWordCloudChart = async () => {
  try {
    ...
    if (!chartDom) {
      console.error('Error: wordCloudChart DOM element not found');
      return;
    }
    ...
    setTimeout(async () => {
      ...
      try {
        // 读取 CSV 文件
        const response = await fetch('/词云图数据.csv');
        const csvText = await response.text();
        ...
        const option = {
          ...
          tooltip: {
            ...
            formatter: function(params) {
              return `${params.name}: ${params.value.toLocaleString()} 个职位`;
            }
          },
          series: [{
            type: 'wordCloud',
        ...
        console.error('Error loading or parsing word cloud CSV:', error);
      }
    }, 100);
  } catch (error) {
    console.error('Error in drawWordCloudChart:', error);
  }
};
...
.hologram-panel {
  ...
  .holo-header {
    ...
    .header-grid {
      ...
      bottom: 0;
      background-image: 
        linear-gradient（rgba(0, 255, 255, 0.1) 1px,
