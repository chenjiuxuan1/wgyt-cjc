...
export class World extends Mini3d {
  ...
  createBar() {
    ...
    data.map((item, index) => {
      // 网格
      let geoHeight = height * (item.value / max)+5
      // 材质
    ...
    function labelStyle04(data, index, position) {
      ...
      <div class="provinces-label-style02-wrap">
         <div class="number">
  <span class="value">${data.value >= 0.5 ? data.value.toFixed(2) : ""}</span>
  <span class="unit">${data.value >= 0.5 ? "万+岗位" : (data.value >= 0 ? "或小于1万的工作岗位" : "")}</span>
</div>
  ...
  clearDataPoints() {
    ...
    if (this.flyLineGroup) {
      this.flyLineGroup.children.forEach(child => {
        if (child.parent) {
          child.parent.remove(child);
        }
      });
      this.flyLineGroup.children = [];
    }
  }

  // 添加数据点
  addDataPoint(data) {
    const { name, position, type, info } = data;
    
    // 创建柱状图
    if (this.currentScene === 'mainScene') {
      const bar = this.createBarPoint(position, type);
      if (bar) {
        this.allBar.push(bar);
      }
    }

    // 创建散点
    const scatter = this.createScatterPoint(position, type);
    if (scatter) {
      this.scatterGroup.add(scatter);
    }

    // 创建标签
    const label = this.createLabelPoint(position, name, info);
    if (label) {
    ...
    // 创建飞线
    if (this.currentScene === 'mainScene') {
      const flyLine = this.createFlyLinePoint(position);
      if (flyLine) {
  ...
  // 创建柱状图点
  createBarPoint(position, type) {
    const [x, y] = position;
    const height = 10; // 默认高度
    ...
    bar.position.set(x, height / 2, y);
    return bar;
  }

  // 创建散点
  createScatterPoint(position, type) {
    const [x, y] = position;
    ...
    const scatter = new Mesh(geometry, material);
    scatter.position.set(x, 0.5, y);
    return scatter;
  }

  // 创建标签点
  createLabelPoint(position, name, info) {
    const [x, y] = position;
    const label = this.label3d.createLabel({
      text: name,
      position: [x, 2, y],
      info: info
    });
  ...
  // 创建飞线点
  createFlyLinePoint(position) {
    const [x, y] = position;
    const start = new Vector3(x, 0, y);
    const end = new Vector3(this.flyLineCenter[0], 0, this.flyLineCenter[1]);
