...
class EmotionVideoAnalyzer:
    ...
    def analyze_video(self):
        ...
        # 使用 Video.analyze 方法分析视频，每隔50帧
        raw_data = video.analyze(
            detector=self.detector,
            frequency=frame_interval,
            display=False,
            save_video=False,
            save_frames=False
        )
        ...
        if raw_data:
            ...
            if not df.empty:
                ...
                emotions_df = video.get_emotions(df)
                
                # 将DataFrame转换为列表形式
                for i, (_, row) in enumerate(emotions_df.iterrows()):
                    frame_number = i * frame_interval
                    results.append({
                        'frame_number': frame_number,
                        'emotions': row.to_dict()
                    })
        ...
        # 如果有情绪结果，也保存为CSV格式
        if raw_data and not emotions_df.empty:
            csv_path = os.path.join(app.config['RESULTS_FOLDER'], f"{filename}.csv")
            emotions_df.to_csv(csv_path)
